    ...
    execute: (args) => {
        ...
        if(args[1] == undefined)
        {
            ...
            .then(res => {
                ...
                ChatLib.chat("")
                ChatLib.chat(`${constants.PREFIX}&bDo '&e/cw import (route)&b' to copy the import for Skytils!`)
            })
            .catch(err => {
                ...
                ChatLib.chat("")
                ChatLib.chat(`${constants.PREFIX}&bDo '&e/cw import (route)&b' to copy the import!`)
            })
        }
        else
        {
            ...
            if(route == undefined)
            {
                ...
                .then(res => {
                    if(res.data.found !== false)
                    {
                        ...
                        ChatLib.chat(`${constants.PREFIX}&bSuccessfully copied &a${args[1]}&b route's data to clipboard! Format: &a${res.data.format}`)
                    }
                    else
                        ChatLib.chat(`${constants.PREFIX}&cCould not find the route '&e${args[1]}&c'!`)
                })
                .catch(err => {
                    ChatLib.chat(`${constants.PREFIX}&cCThere was an error fetching the api! ${err}`)
                })
            }
            else
            {
                ...
                ChatLib.chat(`${constants.PREFIX}&bSuccessfully ${route.format != "soopy" ? "copied" : "loaded"} &a${args[1]}&b route! Format: &a${route.format}`)
            }
        }
    }
})
