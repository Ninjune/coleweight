import { PREFIX, load, registerCommands } from "../../utils/Utils"
import settings from "../../settings"
import { Title, drawCoolWaypoint, drawLine, trace } from "../../utils/Rendering"
    ...
    execute: (args) => {
        ...
        switch(args[1].toLowerCase()) {
            ...
            case "clear":
                ...
                lastCloser = 0
                ChatLib.chat(`${PREFIX}&bUnloaded ordered waypoints!`)
                break
            case "skip":
                if(args[2] != undefined && parseInt(args[2]) != args[2])
                    return ChatLib.chat(`${PREFIX}&bNot an integer!`)
                ...
                ChatLib.chat(`${PREFIX}&bSkipped ${args[2] ?? 1} vein(s).`)
                break
            case "skipto":
                if(parseInt(args[2]) > 0 && parseInt(args[2]) < orderedWaypoints.length)
                {
                    ...
                    ChatLib.chat(`${PREFIX}&bSkipped to ${parseInt(args[2])}`)
                }
                else
                ChatLib.chat(`${PREFIX}&eSkipto must be in range 1 - ${orderedWaypoints.length-1}`)
                break
            case "unskip":
                if(args[2] != undefined && parseInt(args[2]) != args[2])
                    return ChatLib.chat(`${PREFIX}&bNot an integer!`)
                ...
                ChatLib.chat(`${PREFIX}&bWent back ${args[2] ?? 1} waypoints.`)
                break
            case "enable":
                enabled = true
                ChatLib.chat(`${PREFIX}&bEnabled ordered waypoints!`)
                renderer.register()
                break
            case "disable":
                enabled = false
                ChatLib.chat(`${PREFIX}&bDisabled ordered waypoints!`)
                renderer.unregister()
                break
            case "delete":
            case "remove":
                if(orderedWaypoints.length < 1) return ChatLib.chat(`${PREFIX}&eWaypoints have not been loaded!`)
                if(args[2] == undefined) return ChatLib.chat(`${PREFIX}&bUsage: '/cw ordered delete (number)'`)
                wNum = parseInt(args[2])
                if(wNum == undefined || wNum < 1 || wNum > orderedWaypoints.length) return ChatLib.chat(`${PREFIX}&eInvalid number! Must be in range (1 - ${orderedWaypoints.length})`)
            ...
            case "add":
            case "insert":
                if(args[2] == undefined) return ChatLib.chat(`${PREFIX}&bStand where you want to add a waypoint (will be block under you) and do '/cw waypoint insert (number)'`)
                ...
                break
            case "etherwarp":
                if(args[2] == undefined) return ChatLib.chat(`${PREFIX}&bMarks a vein as etherwarp. Usage: '/cw ordered etherwarp (number)'`)
                wNum = parseInt(args[2])-1
                if(orderedWaypoints[wNum] == undefined) return ChatLib.chat(`${PREFIX}&bVein does not exist.`)

                orderedWaypoints[wNum].options.ether = !(orderedWaypoints[wNum]?.options?.ether ?? false)
                ChatLib.chat(`${PREFIX}&bWaypoint ${wNum+1} is now ${orderedWaypoints[wNum].options.ether ? "enabled" : "disabled"} to etherwarp.`)

                break
            case "paneclip":
                if(args[2] == undefined) return ChatLib.chat(`${PREFIX}&bMarks a vein as paneclip. Usage: '/cw ordered paneclip (number)'`)
                wNum = parseInt(args[2])-1
                if(orderedWaypoints[wNum] == undefined) return ChatLib.chat(`${PREFIX}&bVein does not exist.`)

                orderedWaypoints[wNum].options.clip = !(orderedWaypoints[wNum]?.options?.clip ?? false)
                ChatLib.chat(`${PREFIX}&bWaypoint ${wNum+1} is now ${orderedWaypoints[wNum].options.ether ? "enabled" : "disabled"} to paneclip.`)
            ...
            case "save":
                if(args[2] == undefined)
                    return ChatLib.chat(`${PREFIX}&bUsage: /cw ordered save (name) [description, can be spaced]`)
...
const renderer = register("renderWorld", () => {
    ...
    for(let i = 0; i < renderWaypoints.length; i++) {
        ...
        if(!orderedWaypoints[renderWaypoints[i]]) {
            if(settings.debug)
