...
export function trackCollection(arg)
{
    ...
    if(collections[arg.toLowerCase()] == undefined) return ChatLib.chat(`${constants.PREFIX}&eThat is not a valid collection! (or is not supported)`)
    collectionGui.trackedItem = collections[arg].collectionToTrack
    collectionGui.itemStringed = collections[arg].collectionStringed

    ChatLib.chat(`${constants.PREFIX}&bSet collection to ${collectionGui.itemStringed}!`)
}
